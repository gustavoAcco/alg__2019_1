...
//exercicio numero 1 lista dia 04/05
static void arrasta_direita (Lista *l, int ind) {
	int i;

	for (i = l->ultimo; i >= ind; i--) {
		l->item [l->ultimo + 1] = l->item [l->ultimo];
	}
}

void adiciona_item_ind (Lista *l, int chave, int ind) {
	if (ind > l->ultimo) {
		...
		if (verifica_lista_cheia (l)) {
			...
			arrasta_direita (l, ind);
			l->item [ind].chave = chave;
			l->ultimo++;
		}
	}
}
